...
import CartContentItem from "../Micro/CartContentItem";
...
export default function Cart () {
    ...
    return <div id="Mob" className=" w-[50vw] dark:text-white dark:bg-slate-900 xl:flex bg-slate-100 font-inter h-screen p-8 gap-10 hidden flex-col ">
       
       <div className="flex flex-row content-center justify-between text-lg font-bold">
           <h1>Added Movies</h1>
              <h1>{`Total : $${calculateTotal()}`}</h1>
       </div>

       <div className="flex  flex-col p-3 h-[60%] border-2 border-gray-300 rounded-sm shadow-sm gap-4 w-full overflow-y-scroll ">
...

            {
                cart.length > 0 ? cart.map((item)=>{
                    ...
                    return <CartContentItem key={Math.random()} name={item.name} item={item.image}  price={item.price.toString()} />
 ...
 </div> : <div className="flex flex-col content-center self-center justify-center text-lg font-bold md:lg:xl:text-xl buttons">
              <p>Cart is Empty </p>
       </div>
       }
    </div>
